...
class SurfDatasetFromCSV(Dataset):
    """
    ...
    def __init__(self, data_csv_filename: str, surf_data_dir: str):
        """
        初始化数据集类，加载CSV数据文件。
        """
        # 读取CSV文件到DataFrame
    ...
    def __getitem__(self, idx):
        ...
        # 读取表面数据，转换为numpy数组
        surf_data_1 = pd.read_csv(surf_filepath_1, header=None).values
        surf_data_2 = pd.read_csv(surf_filepath_2, header=None).values
        ...
        para_data = torch.tensor(
            self.data_frame.iloc[idx, 2:-2].values.astype(float), dtype=torch.float32
        ).unsqueeze(0)

        # 读取预测目标数据，转换为张量 (真实接触面积, 接触热阻)
        target_data = torch.tensor(
            self.data_frame.iloc[idx, -2:].values.astype(float), dtype=torch.float32
        ).squeeze(0)

        # 返回表面数据、参数数据和目标数据
        return surf_data, para_data, target_data
    
class SurfDatasetFromMat(Dataset):
    """
    定义一个数据集类，用于处理和加载表面相关的数据。
    ...
    def __getitem__(self, idx):
        ...
        # 读取表面数据，转换为numpy数组
        surf_data_1 = loadmat(surf_filepath_1)["Z1"]
        surf_data_2 = loadmat(surf_filepath_2)["Z2"]
